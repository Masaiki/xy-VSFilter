...
function Get-LibassVersionFromVcpkg([string]$vcpkgRoot, [string]$vcpkgInstalledDir, [string]$platform, [string]$explicitTriplet) {
    ...
    $triplets = Get-VcpkgTriplets $platform $explicitTriplet
    foreach ($triplet in $triplets) {
        ...
        $shareDir = Join-Path $vcpkgInstalledDir "$triplet\share\libass"
        if (Test-Path $shareDir) {
            $copyrightFile = Join-Path $shareDir "copyright"
            if (Test-Path $copyrightFile) {
                $copyrightText = Get-Content $copyrightFile -Raw
                if ($copyrightText -match '(?m)^Version:\s*([^\r\n]+)') {
                    return $Matches[1].Trim()
                }
            }
        }
    ...
    foreach ($shareDir in Get-ChildItem -Path $vcpkgInstalledDir -Directory | ForEach-Object { Join-Path $_.FullName "share\libass" }) {
        if (Test-Path $shareDir) {
            $statusFile = Join-Path $vcpkgInstalledDir "vcpkg\status"
            if (Test-Path $statusFile) {
                $statusText = Get-Content $statusFile -Raw
                $triplet = Split-Path (Split-Path $shareDir -Parent) -Leaf
                foreach ($paragraph in [regex]::Split($statusText, "\r?\n\r?\n")) {
                    if ($paragraph -match '(?m)^Package:\s+libass\s*$' -and
                        $paragraph -match "(?m)^Architecture:\s+$([regex]::Escape($triplet))\s*$" -and
                        $paragraph -match '(?m)^Version:\s+([^\r\n]+)') {
...
if (-not $vcpkgLibassVersion) {
    foreach ($assHeader in $assHeaderCandidates) {
        if (-not (Test-Path $assHeader)) {
            continue
        }

        $assHeaderText = Get-Content $assHeader -Raw
        if ($assHeaderText -match '#define\s+LIBASS_VERSION_STRING\s+"([^"]+)"') {
            $libassVersion = $Matches[1]
            break
        }

        if ($assHeaderText -match '#define\s+LIBASS_VERSION\s+0x([0-9A-Fa-f]{8,})') {
            $hex = $Matches[1].PadLeft(8, "0")
            $libassMajor = [Convert]::ToInt32($hex.Substring($hex.Length - 8, 2), 16)
            $libassMinor = [Convert]::ToInt32($hex.Substring($hex.Length - 6, 2), 16)
            $libassMicro = [Convert]::ToInt32($hex.Substring($hex.Length - 4, 2), 16)
            $libassVersion = "$libassMajor.$libassMinor.$libassMicro"
            break
        }
    }
}
