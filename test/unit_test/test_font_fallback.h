...
TEST(FontFallbackTest, NormalizesInvalidModesToAuto)
{
    EXPECT_EQ(TEXT_RENDERER_LEGACY_GDI, NormalizeTextRendererMode(TEXT_RENDERER_LEGACY_GDI));
    EXPECT_EQ(TEXT_RENDERER_AUTO_FALLBACK, NormalizeTextRendererMode(-1));
    EXPECT_EQ(TEXT_RENDERER_AUTO_FALLBACK, NormalizeTextRendererMode(TEXT_RENDERER_MODE_COUNT));
}
